...
export class MonitoringTugasAkhirComponent {
  ...
  ngOnInit() {
    ...
    this.formDetailMonitoringSkripsi =
      this.addDetailMonitoringSkripsiFormGroup();
  }
  ...
  submitFormMonitoringSkripsi() {
    ...
    if (
      this.formMonitoringSkripsi.controls['id'].value == null ||
      this.formMonitoringSkripsi.controls['id'].value == ''
    ) {
  ...
  onSubmitAddMonitoringSkripsi(
    formMonitoringSkripsi: Pick<
      MonitoringTugasAkhir,
      'idDosenPembimbing1' | 'idDosenPembimbing2' | 'judulSkripsi'
    >
  ): void {
    this.monitoringTugasAkhirService
      .create(formMonitoringSkripsi, this.decodedToken.userId)
      .subscribe(() => {
  ...
  submitFormDetailMonitoringSkripsi() {
    ...
    if (
      this.formDetailMonitoringSkripsi.controls['id'].value == null ||
      this.formDetailMonitoringSkripsi.controls['id'].value == ''
    ) {
      this.onSubmitAddDetailMonitoringSkripsi(
        this.formDetailMonitoringSkripsi.value
      );
    }
  }

  onSubmitAddDetailMonitoringSkripsi(
    formDetailMonitoringSkripsi: Pick<
      MonitoringTugasAkhir,
      'idMonitoringSkripsi' | 'tanggalMonitoring' | 'uraianKegiatan'
    >
  ): void {
    console.log(
      'idmonitoringtugasakhir : ' +
        formDetailMonitoringSkripsi.idMonitoringSkripsi
    );
    this.detailMonitoringTugasAkhirService
      .create(formDetailMonitoringSkripsi)
      .subscribe(() => {
  ...
  loadJudulSkripsiByUserId() {
    this.monitoringTugasAkhirService
      ...
      .subscribe((res) => {
        ...
        this.detailMonitoringTugasAkhirService
          .fetchByIdMonitoring(this.tugasAkhir[0].id)
          .subscribe((res) => {
  ...
  downloadPDF() {
    ...
    doc.text('Lembar Monitoring Mahasiswa', 14, 15);

    autoTable(doc, {
      html: this.pdfTable.nativeElement,
      startY: 20,
      useCss: true, // agar mengikuti style tabel
    });

    doc.save('lembar-monitoring-mahasiswa.pdf');
  }
}
